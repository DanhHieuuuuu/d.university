...
const ReportDetailModal: React.FC<ReportDetailModalProps> = ({ isModalOpen, setIsModalOpen, report, onClose }) => {
      ...
      return (
        <div style={{ marginTop: 8, backgroundColor: '#f9f9f9', padding: '8px 12px', borderRadius: 6 }}>
          ...
          {hasResponses ? (
            ...
            <div style={{ fontStyle: 'italic', color: '#999', fontSize: 13, marginTop: 4 }}>
  ...
  return (
    ...
    >
      <Form form={form} layout="vertical">
          ...
          <Col span={8}>
            <Form.Item label="Ngày tạo báo cáo" name="lastGenerated" style={{ marginBottom: 0 }}>
              <DatePicker
                ...
                disabled
                style={{ width: '100%', backgroundColor: 'transparent', color: '#000' }}
              />
            </Form.Item>
          </Col>
        </Row>
          ...
          renderItem={(item, index) => (
            <List.Item>
              <Card
                ...
                extra={
                  <Tag color={item.type === 3 ? 'purple' : 'blue'}>{item.type === 3 ? 'Tự luận' : 'Trắc nghiệm'}</Tag>
                }
        ...
        <Divider orientation="left" style={{ borderColor: '#d9d9d9', marginTop: 24 }}>
          Danh sách người trả lời ({report?.respondents?.length || 0})
        </Divider>
        <List
          size="small"
          bordered
          ...
          renderItem={(item, index) => (
            <List.Item>
              <List.Item.Meta
                ...
                description={
                  <span>
                    Mã: {item.userCode} | Điểm: <Tag color="gold">{item.totalScore}</Tag>
                  </span>
                }
              />
              <div style={{ fontSize: 12, color: '#888' }}>{dayjs(item.submitTime).format('DD/MM/YYYY HH:mm')}</div>
            </List.Item>
          )}
