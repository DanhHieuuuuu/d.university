...
const PositionModal: React.FC<PositionModalProps> = ({
  ...
  useEffect(() => {
    if (
      $create.status === ReduxStatus.SUCCESS ||
      $update.status === ReduxStatus.SUCCESS
    ) {
  ...
  const handleFinish: FormProps['onFinish'] = async (values) => {
    const payload = {
      ...values,
      idNhanSu:
        typeof values.idNhanSu === 'object'
          ? values.idNhanSu.value
          : values.idNhanSu,
    };
    ...
    try {
      if (isUpdate && $selected.data) {
        await dispatch(
          updateKpiRole({ id: $selected.data.id, ...payload })
        ).unwrap();
