...
import { deleteToBoMon, getAllToBoMon } from '@redux/feature/danh-muc/danhmucThunk';
...
const Page = () => {
  ...
  const { query, pagination, onFilterChange, resetFilter } = usePaginationWithFilter<IQueryToBoMon>({
    total: totalItem || 0,
    initialQuery: {
      PageIndex: 1,
      PageSize: 10,
  ...
  const onClickDelete = (id: number) => {
    ...
    _modal.confirm({
      type: 'warning',
      title: 'Bạn chắc chắn muốn xóa tổ bộ môn này ?',
      cancelText: 'Hủy',
      okText: 'Xác nhận',
      onOk: () => {
        var result = dispatch(deleteToBoMon(id))
          .unwrap()
          .then(() => {
            refreshData();
          });
