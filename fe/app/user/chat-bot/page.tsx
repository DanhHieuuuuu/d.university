...
function ChatbotPage() {
    ...
    const renderChatbot = () => (
        <div className="flex h-screen" style={{ backgroundColor: colors.grayLight }}>
            ...
            {/* Sidebar */}
            <div
                className={`fixed md:relative z-40 h-full transition-transform duration-300 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'
                    }`}
                style={{
                    width: 280,
                ...
                <div className="flex flex-col h-full">
                    ...
                    <div className="flex-1 overflow-y-auto px-2">
                        ...
                        {isLoadingSessions ? (
                            ...
                            <p className="px-2 py-4 text-sm text-center" style={{ color: colors.gray }}>
                            ...
                            sessions.map((session: IChatSession) => (
                                <div
                                    ...
                                    className={`group flex items-center justify-between px-3 py-2 mb-1 rounded-lg cursor-pointer transition-colors ${activeSessionId === session.session_id ? 'bg-blue-50' : 'hover:bg-gray-100'
                                        }`}
                                    style={{
                    ...
                    <div className="p-4 border-t" style={{ borderColor: colors.grayLight }}>
                        {user && (
                            <div className="mb-3 text-sm" style={{ color: colors.black }}>
                                <p className="font-medium">{user.hoDem && user.ten ? `${user.hoDem} ${user.ten}` : (user.ten || 'Sinh vien')}</p>
            ...
            <div className="flex-1 flex flex-col h-full md:ml-0">
                ...
                >
                    <h2 className="text-lg font-semibold pl-12 md:pl-0" style={{ color: colors.primaryNavy }}>
                ...
                <div className="flex-1 overflow-y-auto p-6">
                    {isLoadingMessages ? (
                        ...
                        activeMessages.map((msg: ChatMessageUI) => (
                            ...
                            >
                                <div
                                    className={`max-w-[70%] px-4 py-3 rounded-2xl ${msg.role === 'assistant' ? 'markdown-content' : ''}`}
                                    style={{
                                ...
                                >
                                    {msg.role === 'assistant' ? (
                                        <ReactMarkdown
                                            ...
                                            components={{
                                                h2: ({ children }) => <h2 style={{ fontSize: '1.25rem', fontWeight: 'bold', marginBottom: '0.75rem', color: colors.primaryNavy }}>{children}</h2>,
                                                h3: ({ children }) => <h3 style={{ fontSize: '1.1rem', fontWeight: '600', marginBottom: '0.5rem', color: colors.primaryNavy }}>{children}</h3>,
                                                p: ({ children }) => <p style={{ marginBottom: '0.5rem', lineHeight: '1.6' }}>{children}</p>,
                                                strong: ({ children }) => <strong style={{ fontWeight: '600', color: colors.primary }}>{children}</strong>,
                                                table: ({ children }) => <table style={{ width: '100%', borderCollapse: 'collapse', marginTop: '0.75rem', marginBottom: '0.75rem', fontSize: '0.9rem' }}>{children}</table>,
                                                thead: ({ children }) => <thead style={{ backgroundColor: colors.primaryLight + '20' }}>{children}</thead>,
                                                th: ({ children }) => <th style={{ padding: '0.5rem 0.75rem', textAlign: 'left', fontWeight: '600', borderBottom: `2px solid ${colors.primary}`, color: colors.primaryNavy }}>{children}</th>,
                                                td: ({ children }) => <td style={{ padding: '0.5rem 0.75rem', borderBottom: `1px solid ${colors.grayLight}` }}>{children}</td>,
                                                blockquote: ({ children }) => <blockquote style={{ borderLeft: `4px solid ${colors.primary}`, paddingLeft: '1rem', marginTop: '0.75rem', marginBottom: '0.75rem', backgroundColor: colors.primaryLight + '10', padding: '0.75rem 1rem', borderRadius: '0 0.5rem 0.5rem 0', fontStyle: 'italic' }}>{children}</blockquote>,
                                                ...
                                                code: ({ children }) => <code style={{ backgroundColor: colors.grayLight, padding: '0.125rem 0.375rem', borderRadius: '0.25rem', fontSize: '0.875rem', fontFamily: 'monospace' }}>{children}</code>,
                                            }}
                                        >
                                            {msg.content}
                                        </ReactMarkdown>
                ...
                {/* Input area */}
                <div className="p-4 border-t" style={{ backgroundColor: colors.white, borderColor: colors.grayLight }}>
