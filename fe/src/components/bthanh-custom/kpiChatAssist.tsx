      ...
      if (line.startsWith('|') && line.endsWith('|')) {
        ...
        if (tableRows.length >= 2) {
          ...
          result.push(
            <div
              key={`tbl-${i}`}
              style={{ overflowX: 'auto', margin: '10px 0', border: '1px solid #d9d9d9', borderRadius: '8px' }}
            >
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px', backgroundColor: '#fff' }}>
                ...
                <tbody>
                  {bodyRows.map((row, rIdx) => (
                    <tr
                      key={rIdx}
                      style={{ borderBottom: rIdx === bodyRows.length - 1 ? 'none' : '1px solid #f0f0f0' }}
                    >
                      {row.map((cell, cIdx) => (
                        <td
                          key={cIdx}
                          style={{
                            padding: '8px 12px',
                            borderRight: cIdx === row.length - 1 ? 'none' : '1px solid #f0f0f0',
                            color: '#595959',
                            verticalAlign: 'top'
                          }}
                        >
        ...
        if (line) {
          if (line.startsWith('* ') || line.startsWith('- ')) {
            ...
            result.push(
              <div
                key={`line-${i}`}
                style={{ minHeight: '22px', marginBottom: '4px', lineHeight: 1.6, whiteSpace: 'pre-wrap' }}
              >
                {formatText(lines[i])}
              </div>
            );
          ...
          {history.length === 0 ? (
            ...
            >
              <Avatar
                size={80}
                icon={<RobotOutlined />}
                style={{ backgroundColor: '#e6f4ff', color: '#1677ff', marginBottom: 16 }}
              />
          ...
          {status === ReduxStatus.LOADING && (
            <div style={{ display: 'flex', gap: 12, marginLeft: 4 }}>
              <Avatar
                ...
                style={{ backgroundColor: '#fff', boxShadow: '0 2px 6px rgba(0,0,0,0.08)' }}
              />
              <div
                style={{
                  backgroundColor: '#fff',
