...
import { IQuerySurvey, IQueryMySurvey, ISubmitSurvey, ICreateSurvey, IUpdateSurvey } from '@models/survey/survey.model';
...
export const getPagingRequest = createAsyncThunk(
    ...
    async (payload: IQueryRequest, { rejectWithValue }) => {
        try {
            ...
            return res.data; // Trả về { items: [], totalItem: 0 }
        } catch (err: any) {
...
export const createSurvey = createAsyncThunk(
    ...
    async (payload: ICreateSurvey, { rejectWithValue }) => {
        try {
            const res = await SurveyService.createSurvey(payload);
            return res;
        } catch (err: any) {
            return rejectWithValue(err?.response?.data || 'Tạo khảo sát thất bại');
        }
    }
);
...
export const openSurveyAction = createAsyncThunk(
    'survey/open-survey',
    async (id: number, { rejectWithValue }) => {
        try {
...
export const closeSurveyAction = createAsyncThunk(
    'survey/close-survey',
    async (id: number, { rejectWithValue }) => {
        try {
