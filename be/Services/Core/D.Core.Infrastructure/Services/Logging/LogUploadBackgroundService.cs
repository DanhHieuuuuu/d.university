...
namespace D.Core.Infrastructure.Services.Logging
{
    public class LogUploadBackgroundService : BackgroundService
    {
            ...
            IConfiguration configuration)
        {
            ...
            _s3FolderPrefix = _configuration.GetValue<string>("LogUpload:S3FolderPrefix", "logs/core-api") ?? "logs/core-api";
            ...
            using (var scope = _serviceScopeFactory.CreateScope())
            {
                ...
                foreach (var filePath in logFiles)
                {
                    try
                    {
                        ...
                        using (var fileStream = SysFile.OpenRead(filePath))
                        {
