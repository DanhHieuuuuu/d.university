...
namespace OpenXMLLibrary.Dtos
{
    public class InputReplaceDto
    {
        ...
        public InputReplaceDto(string findText, string replateText)
        {
            FindText = findText;
        ...
        /// </summary>
        /// <param name="findText"></param>
        public InputReplaceDto(string findText)
        {
            FindText = findText;
            ReplaceText = "";
        }
        ...
        public InputReplaceDto(string findText, DateTime? date, string toStringPattern)
        {
            FindText = findText;
        ...
        public InputReplaceDto(string findText, string replateText, EnumReplaceTextFormat typeFormat)
        {
            FindText = findText;
            //viết switch case kiểu mới
            ReplaceText = typeFormat switch
            {
                EnumReplaceTextFormat.TitleCase => CultureInfo.CurrentCulture.TextInfo.ToTitleCase(replateText?.ToLower() ?? " "),
        ...
        /// <param name="typeFormat"></param>
        public InputReplaceDto(string findText, double? replateNumber, EnumReplaceTextFormat typeFormat)
        {
            FindText = findText;
            //viết switch case kiểu mới
            if (replateNumber != 0)
            {
                ReplaceText = typeFormat switch
                {
                    EnumReplaceTextFormat.NumberVietNam => replateNumber?.ToString("#,#.#", new CultureInfo("is-IS")) ?? "",
                    EnumReplaceTextFormat.NumberToWord => NumberToText.ConvertNumberToText(replateNumber ?? 0) ?? "",
                    _ => ""
                };
            }
            else
        ...
        public InputReplaceDto(string findText, Stream replaceImage, string replaceImageExtension, double replaceImageWidth, double replaceImageHeight)
        {
            FindText = findText;
        ...
        /// <param name="replaceImage"></param>
        /// <param name="replaceImageExtension"></param>
        public InputReplaceDto(string findText, Stream replaceImage, string replaceImageExtension)
        {
            FindText = findText;
            ReplaceImage = replaceImage;
            ReplaceImageExtension = replaceImageExtension;
